    ...
    <div className="flex-1 overflow-hidden flex flex-col min-h-0 pt-4">
      ...
      <div className="flex items-center justify-end gap-3 mb-4 shrink-0">
        <Button
          ...
          className="h-9 px-4 rounded-xl border-slate-200 text-slate-600 font-semibold shadow-sm hover:bg-slate-50 flex items-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8 shrink-0">
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-100 dark:border-slate-800/60 rounded-2xl p-6 shadow-sm flex items-center justify-between">
          <div>
            <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5">Total Reviews</div>
            <div className="text-4xl font-extrabold text-slate-800 dark:text-slate-100">{totalHistoryCount}</div>
          </div>
        ...
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-100 dark:border-slate-800/60 rounded-2xl p-6 shadow-sm flex items-center justify-between">
          <div>
            <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5">Average Score</div>
            <div className="text-4xl font-extrabold text-slate-800 dark:text-slate-100">{avgScore}</div>
          </div>
        ...
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-100 dark:border-slate-800/60 rounded-2xl p-6 shadow-sm flex items-center justify-between">
          <div>
            <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5">Total Time</div>
            <div className="text-3xl font-extrabold text-slate-800 dark:text-slate-100">{totalDurFormatted}</div>
          </div>
        ...
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-100 dark:border-slate-800/60 rounded-2xl p-6 shadow-sm flex items-center justify-between">
          <div>
            <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5">Average Time</div>
            <div className="text-3xl font-extrabold text-slate-800 dark:text-slate-100">{avgDurFormatted}</div>
          </div>
      ...
      <div className="flex-1 overflow-auto bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-2xl border border-white/40 dark:border-slate-800/40 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-1">
        <Table>
          <TableHeader>
            <TableRow className="border-b border-slate-100 dark:border-slate-800 hover:bg-transparent">
              ...
              <TableHead className="font-semibold text-slate-500">Submit Date</TableHead>
              <TableHead className="font-semibold text-slate-500">Start Time</TableHead>
              <TableHead className="font-semibold text-slate-500">Created Date</TableHead>
              <TableHead className="font-semibold text-slate-500">Ticket ID</TableHead>
              <TableHead className="font-semibold text-slate-500">Solusi</TableHead>
              <TableHead className="font-semibold text-slate-500">Tagging</TableHead>
              ...
              <TableHead className="font-semibold text-slate-500">Team Leader</TableHead>
              <TableHead className="font-semibold text-slate-500">
                <div className="flex items-center">
                  Tapper
              ...
              <TableHead className="font-semibold text-slate-500">Agent AHT</TableHead>
              <TableHead className="font-semibold text-slate-500">QC ATT</TableHead>
              <TableHead className="font-semibold text-slate-500">Pause Reasons</TableHead>
              <TableHead className="font-semibold text-slate-500">Score</TableHead>
          ...
          <TableBody className={cn("transition-opacity duration-200", isFetchingHistory ? "opacity-50 pointer-events-none" : "opacity-100")}>
            {history.length === 0 && !isFetchingHistory ? (
              <TableRow>
                <TableCell colSpan={17} className="h-[300px]">
                  <div className="flex flex-col items-center justify-center text-center h-full">
                    ...
                    <h3 className="text-lg font-bold text-slate-700 dark:text-slate-300 mb-2">No review history found</h3>
                    <p className="text-sm text-slate-500 max-w-[250px] mb-6">Complete an evaluation in the Pending Tickets tab and your history will appear here.</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
              history.map((h: any, i: number) => {
                ...
                return (
                  <TableRow key={h.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors border-b border-slate-50 dark:border-slate-800/50">
                    <TableCell className="text-slate-400 font-medium">{(historyPage - 1) * itemsPerPage + i + 1}</TableCell>
                    ...
                    <TableCell className="text-slate-500">{h.createdDate ? new Date(h.createdDate).toLocaleDateString("id-ID") : "-"}</TableCell>
                    <TableCell className="font-bold text-slate-900 dark:text-white">{h.idTiket}</TableCell>
                    <TableCell className="text-slate-600 dark:text-slate-300 min-w-[150px]">{h.solusi?.replace(/ \| /g, ", ") || "-"}</TableCell>
                    <TableCell className="text-slate-600 dark:text-slate-300 whitespace-nowrap">{h.tagging || "-"}</TableCell>
                    <TableCell className="text-slate-700 dark:text-slate-300 font-medium">{h.agent}</TableCell>
                    <TableCell className="text-slate-500">{h.teamLeader || '-'}</TableCell>
                    <TableCell className="text-slate-500">{h.tapper}</TableCell>
                    <TableCell className="text-slate-500">{h.channel || '-'}</TableCell>
                    <TableCell className="text-slate-500">{h.jenisInteraksi || '-'}</TableCell>
                    <TableCell className="text-slate-500">{h.kipLevel2 || '-'}</TableCell>
                    <TableCell className="text-slate-500">{h.kipLevel3 || '-'}</TableCell>
                    <TableCell className="text-slate-500">{h.handlingTime || '-'}</TableCell>
                    <TableCell className="text-slate-500 font-mono text-xs">{formatTime(h.tappingDuration || 0)}</TableCell>
                    {/* Pause Reasons */}
                    <TableCell className="text-slate-500 min-w-[180px]">
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Button 
                          ...
                          className="h-9 w-9 p-0 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-colors"
                        >
        ...
        <div className="flex items-center justify-between p-4 border-t border-slate-100 dark:border-slate-800/50">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="h-9 rounded-lg border-slate-200 text-slate-600 shadow-sm" onClick={() => setHistoryPage(p => Math.max(1, p - 1))} disabled={historyPage === 1}>
            ...
            <div className="flex items-center gap-1">
              {getPaginationRange(historyPage, totalHistoryPages).map((pageNumber, idx) => (
                <Button
                  ...
                  size="sm"
                  className={cn("w-9 h-9 rounded-lg", historyPage === pageNumber ? "bg-slate-900 text-white shadow-md" : "text-slate-600", pageNumber === "..." ? "pointer-events-none" : "")}
            ...
            <Button variant="outline" size="sm" className="h-9 rounded-lg border-slate-200 text-slate-600 shadow-sm" onClick={() => setHistoryPage(p => Math.min(totalHistoryPages, p + 1))} disabled={historyPage === totalHistoryPages}>
        ...
        <AlertDialogContent className="bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-500">
              Tindakan ini akan menghapus <span className="font-bold text-slate-800 dark:text-slate-200">{totalHistoryCount} riwayat</span> secara permanen dan tidak dapat dibatalkan.
