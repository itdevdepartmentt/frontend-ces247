...
export default function EvaluateTicketPage() {
  ...
  if (isLoading) {
    return (
      <div className="h-full flex items-center justify-center bg-zinc-50 dark:bg-zinc-950 min-h-[500px]">
  ...
  if (isError || !currentTicket) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-zinc-50 dark:bg-zinc-950 min-h-[500px]">
        <h2 className="text-xl font-bold text-zinc-800 dark:text-zinc-200 mb-4">Ticket not found</h2>
  ...
  return (
    <div className="h-full bg-white dark:bg-zinc-950 flex flex-col overflow-hidden relative">
      <div className="w-full flex flex-col h-full overflow-hidden relative z-10">
        <div className="px-6 py-5 sm:px-8 border-b border-zinc-100 dark:border-zinc-800/60 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between bg-transparent shrink-0">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" onClick={() => router.push("/quality-assurance/form-tapping")} className="rounded-full w-10 h-10 bg-zinc-100 dark:bg-zinc-900 hover:bg-zinc-200 dark:hover:bg-zinc-800">
              <ArrowLeft className="w-5 h-5 text-zinc-600 dark:text-zinc-400" />
            </Button>
            <h1 className="text-xl font-bold flex items-center gap-3 text-zinc-900 dark:text-white">
              Evaluate Ticket
              <span className="bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 px-3 py-1 rounded-full text-xs font-semibold tracking-wider font-mono border border-zinc-200 dark:border-zinc-700">{currentTicket.idTiket}</span>
            </h1>
          </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col sm:flex-row bg-transparent relative">
            ...
            <div className="p-6 sm:p-8 space-y-10">
              <section>
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xs font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">Ticket Metadata</h3>
                  <Button variant="ghost" size="sm" onClick={() => setIsEditingForm(!isEditingForm)} className={cn("h-8 px-3 rounded-lg font-bold text-xs shadow-sm transition-all border", isEditingForm ? "bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-100" : "bg-white text-zinc-600 border-zinc-200 hover:bg-zinc-50")}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8">
                  ...
                  <div className="flex flex-col gap-2.5">
                    ...
                    {isEditingForm ? (
                      <Input value={formData.agent || ""} onChange={(e) => handleInputChange("agent", e.target.value)} className="h-11 bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 font-medium text-zinc-800 dark:text-zinc-200" />
                    ) : (
                      <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">{formData.agent || "-"}</p>
                    )}
                  </div>
                  <div className="flex flex-col gap-2.5">
                    <Label className="text-[11px] text-zinc-400 font-bold uppercase tracking-widest">Channel</Label>
                    {isEditingForm ? (
                      <Input value={formData.channel || ""} onChange={(e) => handleInputChange("channel", e.target.value)} className="h-11 bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 font-medium text-zinc-800 dark:text-zinc-200" />
                    ) : (
                  ...
                  <div className="flex flex-col gap-2.5">
                    <Label className="text-[11px] text-zinc-400 font-bold uppercase tracking-widest">KIP Level 2</Label>
                    {isEditingForm ? (
                      <Input value={formData.kipLevel2 || ""} onChange={(e) => handleInputChange("kipLevel2", e.target.value)} className="h-11 bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 font-medium text-zinc-800 dark:text-zinc-200" />
                    ) : (
                      <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">{formData.kipLevel2 || "-"}</p>
                    )}
                  </div>
                  <div className="flex flex-col gap-2.5">
                    <Label className="text-[11px] text-zinc-400 font-bold uppercase tracking-widest">KIP Level 3</Label>
                    {isEditingForm ? (
                      <Input value={formData.kipLevel3 || ""} onChange={(e) => handleInputChange("kipLevel3", e.target.value)} className="h-11 bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 font-medium text-zinc-800 dark:text-zinc-200" />
                    ) : (
                      <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">{formData.kipLevel3 || "-"}</p>
                    )}
                  </div>
                  <div className="flex flex-col gap-2.5">
                    <Label className="text-[11px] text-zinc-400 font-bold uppercase tracking-widest">In/Out SLA</Label>
                    {isEditingForm ? (
                  ...
                  <div className="flex flex-col gap-2.5">
                    <Label className="text-[11px] text-zinc-400 font-bold uppercase tracking-widest">AHT</Label>
                    {isEditingForm ? (
                      <Input value={formData.handlingTime || ""} onChange={(e) => handleInputChange("handlingTime", e.target.value)} className="h-11 bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 font-medium text-zinc-800 dark:text-zinc-200" />
                    ) : (
                      <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">{formData.handlingTime || "-"}</p>
                    )}
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-6">
                <div className="flex flex-col gap-3">
                  <h3 className="text-xs font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest flex items-center gap-2"><User className="w-4 h-4" /> Customer Request</h3>
                  <div className="relative">
                    <div className="absolute top-0 left-0 w-1.5 h-full bg-blue-500 rounded-l-2xl z-10 pointer-events-none"></div>
                    {isEditingForm ? (
                      <Textarea value={formData.customerRequests || ""} onChange={(e) => handleInputChange("customerRequests", e.target.value)} className="pl-6 bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 rounded-2xl min-h-[140px] shadow-sm resize-y text-sm font-medium leading-relaxed p-5" placeholder="Customer request details..." />
                    ) : (
                      <div className="pl-6 bg-gradient-to-br from-white to-zinc-50/50 dark:from-zinc-900 dark:to-zinc-900/50 border border-zinc-200/60 dark:border-zinc-800 rounded-2xl p-6 text-sm text-zinc-800 dark:text-zinc-200 leading-relaxed min-h-[140px] shadow-sm overflow-hidden whitespace-pre-wrap">
                        {formData.customerRequests || <span className="italic text-zinc-400">No customer request provided</span>}
                      </div>
                    )}
                  </div>
                </div>
                <div className="flex flex-col gap-3">
                  <h3 className="text-xs font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest flex items-center gap-2"><Headset className="w-4 h-4" /> Agent Response</h3>
                  <div className="relative">
                    <div className="absolute top-0 left-0 w-1.5 h-full bg-emerald-500 rounded-l-2xl z-10 pointer-events-none"></div>
                    {isEditingForm ? (
                      <Textarea value={formData.agentResponse || ""} onChange={(e) => handleInputChange("agentResponse", e.target.value)} className="pl-6 bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 rounded-2xl min-h-[140px] shadow-sm resize-y text-sm font-medium leading-relaxed p-5" placeholder="Agent response details..." />
                    ) : (
                      <div className="pl-6 bg-gradient-to-br from-white to-zinc-50/50 dark:from-zinc-900 dark:to-zinc-900/50 border border-zinc-200/60 dark:border-zinc-800 rounded-2xl p-6 text-sm text-zinc-800 dark:text-zinc-200 leading-relaxed min-h-[140px] shadow-sm overflow-hidden whitespace-pre-wrap">
                        {formData.agentResponse || <span className="italic text-zinc-400">No agent response provided</span>}
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="w-full sm:w-1/2 flex flex-col overflow-y-auto relative bg-transparent">
            ...
            <div className="p-6 sm:p-8 space-y-10">
              <section>
                <h3 className="text-xs font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-6">Evaluation Criteria</h3>
                <div className="space-y-6">
                  
                  <div className="flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                      <Label className="font-bold text-zinc-700 dark:text-zinc-300">Validitas Closing Case</Label>
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded">Max 30 pts</span>
                    </div>
                    <div className="flex gap-2 p-1.5 bg-zinc-100/80 dark:bg-zinc-800/80 rounded-xl border border-zinc-200/50 dark:border-zinc-700/50 shadow-inner">
                      <button onClick={() => setScoreValiditas(30)} className={cn("flex-1 h-10 rounded-lg text-sm font-bold transition-all duration-200", scoreValiditas === 30 ? "bg-white dark:bg-zinc-700 shadow text-emerald-600 dark:text-emerald-400" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200")}>Pass (30)</button>
                      <button onClick={() => setScoreValiditas(0)} className={cn("flex-1 h-10 rounded-lg text-sm font-bold transition-all duration-200", scoreValiditas === 0 ? "bg-white dark:bg-zinc-700 shadow text-rose-600 dark:text-rose-400" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200")}>Fail (0)</button>
                    </div>
                  </div>

                  <div className="flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                      <Label className="font-bold text-zinc-700 dark:text-zinc-300">Service Level Closing</Label>
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded">Max 30 pts</span>
                    </div>
                    <div className="flex gap-2 p-1.5 bg-zinc-100/80 dark:bg-zinc-800/80 rounded-xl border border-zinc-200/50 dark:border-zinc-700/50 shadow-inner">
                      <button onClick={() => setScoreServiceLevel(30)} className={cn("flex-1 h-10 rounded-lg text-sm font-bold transition-all duration-200", scoreServiceLevel === 30 ? "bg-white dark:bg-zinc-700 shadow text-emerald-600 dark:text-emerald-400" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200")}>Pass (30)</button>
                      <button onClick={() => setScoreServiceLevel(0)} className={cn("flex-1 h-10 rounded-lg text-sm font-bold transition-all duration-200", scoreServiceLevel === 0 ? "bg-white dark:bg-zinc-700 shadow text-rose-600 dark:text-rose-400" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200")}>Fail (0)</button>
                    </div>
                  </div>

                  <div className="flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                      <Label className="font-bold text-zinc-700 dark:text-zinc-300">Penyampaian Kalimat</Label>
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded">Max 10 pts</span>
                    </div>
                    <div className="flex gap-2 p-1.5 bg-zinc-100/80 dark:bg-zinc-800/80 rounded-xl border border-zinc-200/50 dark:border-zinc-700/50 shadow-inner">
                      <button onClick={() => setScoreKalimat(10)} className={cn("flex-1 h-10 rounded-lg text-sm font-bold transition-all duration-200", scoreKalimat === 10 ? "bg-white dark:bg-zinc-700 shadow text-emerald-600 dark:text-emerald-400" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200")}>Pass (10)</button>
                      <button onClick={() => setScoreKalimat(0)} className={cn("flex-1 h-10 rounded-lg text-sm font-bold transition-all duration-200", scoreKalimat === 0 ? "bg-white dark:bg-zinc-700 shadow text-rose-600 dark:text-rose-400" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200")}>Fail (0)</button>
                    </div>
                  </div>

                  <div className="flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                      <Label className="font-bold text-zinc-700 dark:text-zinc-300">Respon Time</Label>
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded">Max 15 pts</span>
                    </div>
                    <div className="flex gap-2 p-1.5 bg-zinc-100/80 dark:bg-zinc-800/80 rounded-xl border border-zinc-200/50 dark:border-zinc-700/50 shadow-inner">
                      <button onClick={() => setScoreResponTime(15)} className={cn("flex-1 h-10 rounded-lg text-sm font-bold transition-all duration-200", scoreResponTime === 15 ? "bg-white dark:bg-zinc-700 shadow text-emerald-600 dark:text-emerald-400" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200")}>Pass (15)</button>
                      <button onClick={() => setScoreResponTime(0)} className={cn("flex-1 h-10 rounded-lg text-sm font-bold transition-all duration-200", scoreResponTime === 0 ? "bg-white dark:bg-zinc-700 shadow text-rose-600 dark:text-rose-400" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200")}>Fail (0)</button>
                    </div>
                  </div>

                  <div className="flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                      <Label className="font-bold text-zinc-700 dark:text-zinc-300">Dokumentasi</Label>
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded">Max 15 pts</span>
                    </div>
                    <div className="flex gap-2 p-1.5 bg-zinc-100/80 dark:bg-zinc-800/80 rounded-xl border border-zinc-200/50 dark:border-zinc-700/50 shadow-inner">
                      ...
                      <button onClick={() => setScoreDokumentasi(0)} className={cn("flex-1 h-10 rounded-lg text-sm font-bold transition-all duration-200", scoreDokumentasi === 0 ? "bg-white dark:bg-zinc-700 shadow text-rose-600 dark:text-rose-400" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200")}>Fail (0)</button>
                    </div>
                  </div>
              ...
              <section className="pt-8 border-t border-zinc-100 dark:border-zinc-800/60 space-y-8">
                <div className="space-y-8">
                  <div className="flex flex-col gap-3">
                    <Label className="font-bold text-zinc-700 dark:text-zinc-300">Sub Parameter Penilaian *</Label>
                    <div className="flex flex-wrap gap-2 mt-1">
                      {["OK", "Validitas Closing Case", "Service Level Closing", "Penyampaian Kalimat", "Respon Time", "Dokumentasi"].map((opt) => (
                        <div key={opt} 
                          className={cn("cursor-pointer px-4 py-2 rounded-xl border text-sm font-semibold transition-all select-none flex items-center gap-2", subParameterPenilaian.includes(opt) ? "bg-zinc-900 text-white border-zinc-900 dark:bg-white dark:text-zinc-900 dark:border-white shadow-md" : "bg-white text-zinc-600 border-zinc-200 hover:border-zinc-300 dark:bg-zinc-900 dark:text-zinc-400 dark:border-zinc-800 dark:hover:border-zinc-700 shadow-sm")}
                          onClick={() => {
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      {["OK", "Proses & Sikap", "Kualitas Solusi Layanan"].map((opt) => (
                        <div key={opt} 
                          className={cn("cursor-pointer px-4 py-2 rounded-xl border text-sm font-semibold transition-all select-none flex items-center gap-2", parameterPenilaian.includes(opt) ? "bg-zinc-900 text-white border-zinc-900 dark:bg-white dark:text-zinc-900 dark:border-white shadow-md" : "bg-white text-zinc-600 border-zinc-200 hover:border-zinc-300 dark:bg-zinc-900 dark:text-zinc-400 dark:border-zinc-800 dark:hover:border-zinc-700 shadow-sm")}
                          onClick={() => {
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="flex flex-col gap-3">
                      <Label className="font-bold text-zinc-700 dark:text-zinc-300">Final Status *</Label>
                      <div className="bg-zinc-100/80 dark:bg-zinc-800/80 p-1.5 rounded-xl flex gap-1 border border-zinc-200/50 dark:border-zinc-700/50 shadow-inner">
                        <button onClick={() => setStatus("Sample")} className={cn("flex-1 h-10 rounded-lg text-sm font-bold transition-all duration-200", status === "Sample" ? "bg-white dark:bg-zinc-700 shadow text-zinc-900 dark:text-white" : "text-zinc-500 hover:text-zinc-900")}>Sample</button>
                  ...
                  <div className="flex flex-col gap-3 pt-2">
                    <Label className="font-bold text-zinc-700 dark:text-zinc-300">Notes QC</Label>
                    <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Provide additional feedback for the agent..." className="min-h-[120px] rounded-2xl border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50 resize-none focus-visible:ring-zinc-400 p-4 leading-relaxed" />
                  </div>
                </div>
              </section>
            </div>
        ...
        <div className="px-6 py-4 sm:px-8 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0 shadow-[0_-15px_40px_-15px_rgba(0,0,0,0.05)] z-30 relative">
          <div className="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-start">
            <div className="flex items-center gap-4">
              <div className="flex flex-col justify-center">
                <span className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest leading-none mb-1.5">Total QA Score</span>
                <div className="flex items-baseline gap-1">
            ...
            <span className="text-xs font-semibold text-zinc-500 hidden sm:flex items-center gap-2 max-w-[200px] leading-relaxed"><ListChecks className="w-4 h-4 shrink-0" /> Review all inputs carefully before saving.</span>
          </div>
          
          <div className="flex gap-3 w-full sm:w-auto">
            <Button variant="outline" className="flex-1 sm:flex-none h-12 px-6 rounded-xl border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 font-bold hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors" onClick={() => router.push("/quality-assurance/form-tapping")}>
              Cancel
            </Button>
